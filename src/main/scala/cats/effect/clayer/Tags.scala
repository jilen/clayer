...
private[clayer] trait Tags {
  ...
  private[clayer] def taggedIsSubtype(left: LightTypeTag, right: LightTypeTag): Boolean =
    left <:< right

  private[clayer] def taggedTagType[A](tagged: Tag[A]): LightTypeTag =
    tagged.tag
   ...
   */
  private[clayer] def taggedGetHasServices[A](t: LightTypeTag): Set[LightTypeTag] =
