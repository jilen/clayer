...
object TerminalRendering {
  ...
  def missingLayersError(
    ...
    val transitiveStrings = transitive
      .map { case (layer, deps) =>
        val depsString = deps
          .map { dep =>
    ...
    val errors =
      List(topLevelString, transitiveStrings)
 ...
 Alternatively, you may add them to the remainder type ascription:
   ...
   ${methodName("provideSome")}[${allMissingTypes
            .map(_.magenta.bold)
            .mkString(" & ")}]
"""
  ...
  def missingLayersCustomError(
    ...
    val missingLayersString = missingLayers.zipWithIndex
      .map { case (layer, index) =>
  ...
  def unusedLayersError(layers: List[String]) = {
    ...
    val layerStrings = layers.zipWithIndex
      .map { case (layer, index) =>
        val i = s"${index + 1}.".faint
        s"$i ${layer.cyan}"
      }
  ...
  def unusedProvideSomeLayersError(types: List[String]) = {
    ...
    val typeStrings = types.zipWithIndex
      .map { case (tpe, index) =>
        val i = s"${index + 1}.".faint
        s"$i ${tpe.magenta}"
      }
  ...
  def ambiguousLayersError(duplicates: List[(String, List[String])]): String = {
    ...
    val duplicatesString = duplicates
      .map { case (tpe, layers) =>
        // each layer numbered on its own line:
        val numberedLayers = layers.zipWithIndex
          .map { case (l, i) =>
            val idx = s"${i + 1}.".faint
            s"$idx ${l.cyan}"
          }
...
""".stripMargin
  ...
  def main(args: Array[String]): Unit = {
    ...
    println(
      unusedLayersError(List("Clock.live", "UserService.live", "Console.test"))
    )
    println(provideSomeNothingEnvError)
    println(
      unusedProvideSomeLayersError(List("java.lang.String", "List[Boolean]"))
    )
    println(
      ambiguousLayersError(
        List(
          ...
          (
            "List[Boolean]",
            List("someBooleansLayer", "booleanLayer1", "booleanLayer2")
          )
        )
      )
    )
  ...
  object LayerWiringError {
    ...
    final case class MissingTransitive(layer: String, deps: List[String])
        extends LayerWiringError

    final case class Circular(layer: String, dependency: String)
        extends LayerWiringError
  }
