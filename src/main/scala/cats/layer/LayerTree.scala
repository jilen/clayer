...
import LayerTree.{ComposeH, ComposeV, Empty, Value}
...
sealed abstract class LayerTree[+A] extends Product with Serializable { self =>
  ...
  def >>>[A1 >: A](that: LayerTree[A1]): LayerTree[A1] =
    ...
    else ComposeV(self, that)

  def ++[A1 >: A](that: LayerTree[A1]): LayerTree[A1] =
    if (self eq Empty) that
    else if (that eq Empty) self
    else ComposeH(self, that)
  ...
  def fold[B](
    ...
    case ComposeH(left, right) =>
      composeH(
        left.fold(z, value, composeH, composeV),
        right.fold(z, value, composeH, composeV)
      )
  ...
  def toSet[A1 >: A]: Set[A1] =
    fold[Set[A1]](Set.empty[A1], Set(_), _ ++ _, _ ++ _)
...
object LayerTree {
  ...
  final case class ComposeH[+A](left: LayerTree[A], right: LayerTree[A])
      extends LayerTree[A]
  final case class ComposeV[+A](left: LayerTree[A], right: LayerTree[A])
      extends LayerTree[A]

  implicit final class LayerComposeIterableOps[A](
      private val self: Iterable[LayerTree[A]]
  ) extends AnyVal {
    def combineHorizontally: LayerTree[A] =
      self.foldLeft[LayerTree[A]](Empty)(_ ++ _)
  }
}
