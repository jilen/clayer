...
final case class Graph[Key, A](
  ...
  def buildComplete(
    ...
    forEach(outputs) { output =>
      getNodeWithOutput[GraphError[Key, A]](
        output,
        error = GraphError.MissingTopLevelDependency(output)
      )
  ...
  private def buildNode(
    ...
    forEach(node.inputs) { output =>
      getNodeWithOutput[GraphError[Key, A]](
        output,
        error = GraphError.missingTransitiveDependency(node, output)
      )
        .flatMap(node => buildNode(node, Set(node)))
    }
      .map(_.distinct.combineHorizontally)
  ...
  private def getNodeWithOutput[E](
      output: Key,
      error: E
  ): Either[::[E], Node[Key, A]] =
  ...
  private def buildNode(
    ...
    forEach(node.inputs) { input =>
      for {
        out <- getNodeWithOutput(
          input,
          error = GraphError.missingTransitiveDependency(node, input)
        )
  ...
  private def forEach[B, C](
      ...
      f: B => Either[::[GraphError[Key, A]], C]
  ): Either[::[GraphError[Key, A]], List[C]] =
    list.foldRight[Either[::[GraphError[Key, A]], List[C]]](Right(List.empty)) {
      (a, b) =>
        (f(a), b) match {
